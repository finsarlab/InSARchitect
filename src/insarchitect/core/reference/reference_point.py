...
def find_valid_pixel(data: np.array, y: int, x: int) -> tuple:
    """
    ...
    """
    print("[bold magenta]Searching nearest valid pixel using distance transform...[/bold magenta]")

    nan_mask = np.isnan(data)

    indices = distance_transform_edt(nan_mask, return_distances=False, return_indices=True) # return (2, rows, cols)

    nearest_y = indices[0, y, x] # 0 for y coordinates
    nearest_x = indices[1, y, x] # 1 for x coordinates
...
def apply_reference(data: np.array, y: int, x: int) -> np.array:
    ...
    """
    print("[bold magenta]Applying reference point...[/bold magenta]")
...
def reference_point(results: dict, lat: float, lon: float, file_type: str):
    for r in results:
        ...
        if not (0 <= y < h and 0 <= x < w):
            print("[bold red]Computed pixel is outside grid[/bold red]")
            sys.exit(1)

        if y == ref_y and x == ref_x:
            print("[bold yellow]Selected reference point is equal to current point. Nothing to do...[/bold yellow]")
            sys.exit(1)
        ...
        if np.isnan(layer[y, x]):
            print("[bold yellow]The point value is NaN[/bold yellow]")

            nearest_y, nearest_x = find_valid_pixel(data=layer, y=y, x=x)
            new_lat = y_first + (nearest_y * y_step)
            new_lon = x_first + (nearest_x * x_step)

            print(f"[bold cyan]Nearest point found at...[/bold cyan]")
            ...
            print(f"[bold magenta]Updating metadata...[/bold magenta]")
        ...
        print(f"[bold green]Writing output file:[/bold green] {path}")
